...
series1 = pd.read_csv('reentrancy_timelime.csv')
print(series1)
date_min = series1['Timestamp'][0]
date_max = series1['Timestamp'][len(series1)-1]

#date_min = series1.index.values[0]
#date_max = series1.index.values[-1]
data = {'Timestamp': [], 'Transactions': []}
for d in pd.date_range(start=date_min, end=date_max):
    t = d.strftime('%Y-%m-%d')
    for i in range(len(series1)):
        if t == series1['Timestamp'][i]:
            print(t)
            print(series1['Transactions'][i])
            data['Timestamp'].append(t)
            data['Transactions'].append(series1['Transactions'][i])
        else:
            data['Timestamp'].append(t)
...
# Choose some nice levels
hack_levels = np.tile([5, 1, 3],
                 int(np.ceil(len(hack_dates)/3)))[:len(hack_dates)]

fig, ax = plt.subplots()
ax.plot('Timestamp', 'Transactions', data=data, linewidth=1)
#ax.stem('Timestamp', 'Transactions', data=data, markerfmt=" ", use_line_collection=True)
